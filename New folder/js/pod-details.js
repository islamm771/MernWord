...
function displayCases(cases) {
  cases.forEach((obj , index) => {
    const markup = `<div class='case'>
                        <img class="img-fluid" src="./img/case-img.png" alt="">
                        <h4>${obj.name}</h4>
...
function PlayOne(index) {
  ...
  let caseName_case_playlist  = document.querySelector('.playlist .case .case-name')
  let podcastName_case_playlist  = document.querySelector('.playlist .case .podcast-name')
...
function PrevMusic(){
  ...
  if(PlayCases.length > 1){
    ...
    PlayCases[index].classList.add("active");
    document.querySelector(".artist").innerHTML = Cases.Podcast_Name_1[index].name
    music.src = musics[index]
    PlayMusic();
  }
}
